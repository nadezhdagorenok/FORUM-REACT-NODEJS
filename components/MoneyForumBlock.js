...
class MoneyForumBlock extends React.PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            dataReady: false,
    ...
    formSubmit = (mes) => {
        ...
        isoFetch("http://localhost:3000/messages", {
            ...
            body: JSON.stringify({type:'flowers', header: mes.header, message:mes.header})
    ...
    loadData = () => {
        isoFetch("http://localhost:3000/messages?type=flowers", {
            method: 'get',
            headers: {
                "Accept": "application/json",
            },
    ...
    render() {
        console.log('Render MoneyForumBlock');
...
export default MoneyForumBlock;
